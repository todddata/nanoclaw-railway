...
export class GmailChannel implements Channel {
  ...
  async connect(): Promise<void> {
    // Try env vars first (Railway), then fall back to credential files (local)
    ...
    if (clientId && clientSecret && refreshToken) {
      ...
      this.oauth2Client.on('tokens', () => {
        logger.debug('Gmail OAuth tokens refreshed (in-memory)');
      });
    } else {
      ...
      if (!fs.existsSync(keysPath) || !fs.existsSync(tokensPath)) {
        logger.warn(
          'Gmail credentials not found. Skipping Gmail channel. Set GMAIL_CLIENT_ID/GMAIL_CLIENT_SECRET/GMAIL_REFRESH_TOKEN or run /add-gmail.',
        );
        return;
      }
...
registerChannel('gmail', (opts: ChannelOpts) => {
  // Accept env vars (Railway) or credential files (local)
  const hasEnvVars =
  ...
  if (!hasEnvVars) {
    ...
    if (
      ...
      logger.warn(
        'Gmail: set GMAIL_CLIENT_ID/GMAIL_CLIENT_SECRET/GMAIL_REFRESH_TOKEN or run /add-gmail',
      );
      return null;
    }
  }
